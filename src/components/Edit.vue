<template>
    <div class="pt-5">
        ...
        <b-form @submit.prevent="handleSubmit(update)" method="post">
        <validation-provider
          name="name"
          :rules="{ required: true }"
...
export default {
    ...
    mounted() {
        // axios.defaults.headers.Authorization = `Bearer ${JwtService.getAccessToken()}`;
        this.$api.get(process.env.VUE_APP_API+'subscriptions/' + this.$route.params.id+'/')
            .then( response => {
